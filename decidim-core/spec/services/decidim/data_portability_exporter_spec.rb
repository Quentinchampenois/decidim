...
module Decidim
  describe DataPortabilityExporter do
    subject { DataPortabilityExporter.new(user, tmp_file_in.path, password) }

    let(:tmp_file_in) { Tempfile.new(["data-portability", ".7z"]) }
    let(:tmp_dir_out) { Dir.mktmpdir("data_portability_exporter_spec") }
    let(:password) { "data-portability.7z>passwd" }
